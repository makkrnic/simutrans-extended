...
void settings_t::rdwr(loadsave_t *file)
{
	...
	if(file->get_version_int() < 86000) {
		...
		if(file->get_version_int()>=101000) {
			...
			if ((file->get_version_int() > 120002 && (file->get_extended_revision() == 0 || file->get_extended_revision() >= 16)) || file->get_extended_version() >= 13)
			{
		...
		if(file->get_version_int()>102000) {
		...
		if(file->get_version_int()>=102003) {
			// network stuff
			random_counter = get_random_seed( );
			file->rdwr_long( random_counter );
			if(  !env_t::networkmode  ||  env_t::server  ) {
				frames_per_second = sim::clamp(env_t::fps, 5u, 100u); // update it on the server to the current setting
		...
		if(  file->get_version_int()>=112001  ) {
		...
		if(  file->get_version_int()>=112003  ) {
		...
		if(  file->get_version_int()>=120002 && file->get_extended_version() == 0 ) {
		...
		if(  file->get_version_int() >=120002 && (file->get_extended_revision() >= 9 || file->get_extended_version() == 0 || file->get_extended_version() >= 13)) {
			file->rdwr_bool(lake);
			file->rdwr_bool(no_trees);
...
void settings_t::parse_simuconf(tabfile_t& simuconf, sint16& disp_width, sint16& disp_height, sint16 &fullscreen, std::string& objfilename)
{
	...
	env_t::fps = sim::clamp( (uint32)contents.get_int( "frames_per_second", env_t::fps ), env_t::min_fps, env_t::max_fps );
	env_t::ff_fps = sim::clamp( (uint32)contents.get_int( "fast_forward_frames_per_second", env_t::ff_fps ), env_t::min_fps, env_t::max_fps );
	env_t::num_threads = sim::clamp( contents.get_int( "threads", env_t::num_threads ), 1, MAX_THREADS );
	...
	factory_arrival_periods = sim::clamp( contents.get_int("factory_arrival_periods", factory_arrival_periods), 1, 16 );
	...
	used_vehicle_reduction  = sim::clamp( contents.get_int("used_vehicle_reduction", used_vehicle_reduction ), 0, 1000 );
...
//			printf("smpy[%d] year=%d money=%lld\n",k,startingmoneyperyear[k].year,startingmoneyperyear[k].money);
	...
	if(  way_height_clearance > 2  &&  way_height_clearance < bounds  ) {
		uint8 new_whc = sim::clamp( way_height_clearance, bounds, (uint8)2 );
