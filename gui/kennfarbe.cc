	...
	image( skinverwaltung_t::color_options->get_image_id(0), player_->get_player_nr() )
{
	...
	for(unsigned i=0;  i<28;  i++) {
		player_color_1[i].init(button_t::box_state, (used_colors1 & (1 << i) ? "X" : ""), scr_coord(cursor.x + (i % (NUMBER_OF_PLAYER_COLOURS/2))*(D_BUTTON_HEIGHT + D_H_SPACE), cursor.y + (i / (NUMBER_OF_PLAYER_COLOURS/2))*(D_BUTTON_HEIGHT + D_V_SPACE)), scr_size(D_BUTTON_HEIGHT, D_BUTTON_HEIGHT));
	...
	for(unsigned i=0;  i<28;  i++) {
		player_color_2[i].init(button_t::box_state, (used_colors2 & (1 << i) ? "X" : ""), scr_coord(cursor.x + (i % (NUMBER_OF_PLAYER_COLOURS/2))*(D_BUTTON_HEIGHT + D_H_SPACE), cursor.y + (i / (NUMBER_OF_PLAYER_COLOURS/2))*(D_BUTTON_HEIGHT + D_V_SPACE)), scr_size(D_BUTTON_HEIGHT, D_BUTTON_HEIGHT));
	...
	set_windowsize( scr_size( D_MARGIN_LEFT + (NUMBER_OF_PLAYER_COLOURS/2)*D_BUTTON_HEIGHT + 13*D_H_SPACE + D_MARGIN_RIGHT, D_TITLEBAR_HEIGHT + cursor.y + D_MARGIN_BOTTOM ) );
}
